...
public class AwsS3ObjectStorage {
    ...
    public Optional<BlobPutter> putBlob(ContainerName containerName, AwsS3AuthConfiguration configuration) {
        return Optional.of(new AwsS3BlobPutter(configuration, containerName, executorService));
    }
    ...
    private static class AwsS3BlobPutter implements BlobPutter {
        ...
        private AwsS3BlobPutter(AwsS3AuthConfiguration configuration, ContainerName containerName, ExecutorService executorService) {
            this.configuration = configuration;
        ...
        public void putDirectly(Blob blob) {
            writeFileAndAct(blob, (file) -> putWithRetry(containerName, configuration, blob, file, FIRST_TRY));
        }
        ...
        public BlobId putAndComputeId(Blob initialBlob, Supplier<BlobId> blobIdSupplier) {
            Consumer<File> putChangedBlob = (file) -> {
                ...
                putWithRetry(containerName, configuration, initialBlob, file, FIRST_TRY);
            };
        ...
        private AmazonS3 getS3Client(AwsS3AuthConfiguration configuration, ClientConfiguration clientConfiguration) {
