...
public class ManageSieveMailetTestCase {
    ...
    public final void testCapability() throws Exception {
        ...
        when(sieveParser.getExtensions()).thenReturn(Lists.newArrayList("a", "b", "c"));
        initializeMailet();
        mail.setAttribute(new Attribute(Mail.SMTP_AUTH_USER_ATTRIBUTE_NAME, AttributeValue.of("test")));
        mailet.service(mail);
        ensureResponseContains("Re: CAPABILITY", "\"SIEVE\" \"a b c\"",
            "\"IMPLEMENTATION\" \"Apache ManageSieve v1.0\"",
    ...
    public final void testGetScript() throws Exception {
        ...
        MimeMessage message = prepareMimeMessage("GETSCRIPT \"" + SCRIPT_NAME.getValue() + "\"");
        Mail mail = createUnauthenticatedMail(message);
        mail.setAttribute(new Attribute(Mail.SMTP_AUTH_USER_ATTRIBUTE_NAME, AttributeValue.of(USER.asString())));
        mailet.service(mail);
    ...
    private Mail createAuthentificatedMail(MimeMessage message) throws Exception {
        Mail mail = createUnauthenticatedMail(message);
        mail.setAttribute(new Attribute(Mail.SMTP_AUTH_USER_ATTRIBUTE_NAME, AttributeValue.of(message.getSender().toString())));
        return mail;
    }
